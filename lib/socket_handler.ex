defmodule Whistle.SocketHandler do
  ...
  def reply_render(state) do
    ...
    vdom_diff =
      Whistle.Html.diff([], state.vdom, new_vdom)
      |> Whistle.Html.serialize_patches()

    handlers =
      Whistle.Html.extract_event_handlers([], new_vdom)
      |> Enum.into(%{})

    {:reply, {:text, vdom_diff}, %{state | handlers: handlers, vdom: new_vdom}}
  end

  def handle({handler, args}, state = %{handlers: handlers, model: model, vdom: vdom}) do
    message =
